                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                        ...
                        if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                            results, last_hidde_state = evaluate(args, model, tokenizer, 'dev')
                            for key, value in results.items():
                                tb_writer.add_scalar('eval_{}'.format(key), value, global_step)
                            results, last_hidde_state = evaluate(args, model, tokenizer, 'test')
                            for key, value in results.items():
...
def evaluate(args, model, tokenizer, set_type='dev', prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                last_hidden_state = outputs[2] # extract last hidden layer : outputs[2][0] // all layer : outputs[2]

                vectors = []
                for last_hidden_vector in last_hidden_state:
                    vectors.append(last_hidden_vector[:,0].cpu().numpy())

                last_hidden_state = vectors
    ...
    return results, last_hidden_state_list
...
def load_and_cache_examples(args, tokenizer, set_type='train', subset='all'):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_odp'.format(
        set_type,
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = odp_convert_examples_to_features(examples,
                                                    tokenizer,
    ...
    if odp_output_mode == "classification":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.long)
    elif odp_output_mode == "regression":
...
def C_encoder():
    ...
    parser.add_argument("--model_name_or_path", default='./output_odp_bert/odp_finetuned', type=str,
                        help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(
                            ALL_MODELS)) # finetuning model use : default='./output_odp_bert/odp_finetuned' or bert-base-uncased
    parser.add_argument("--output_dir", default='./output_odp_bert/odp_finetuned', type=str,
    ...
    parser.add_argument("--num_train_epochs", default=130.0, type=float,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, last_hidden_state_list = evaluate(args, model, tokenizer, set_type='test', prefix=prefix)
...
if __name__ == "__main__":
    ...
    with open('/hdd/user4/bert_classification/last_hidden/last_hidden_%s-%s-%s-%s-%s.bin' % (dt.year, dt.month, dt.day, dt.hour, dt.minute),'wb') as f:
        pickle.dump(last_hidden, f)



# with open('/hdd/user4/bert_classification/last_hidden/last_hidden_2020-1-22-15-7.bin','rb') as f:
#     import pickle
#     last_hidden = pickle.load(f)


# test = []
# for batches in last_hidden:
...
# import csv
#
# f = open('/hdd/user4/bert_classification/agnews_data/ag_news_test.csv', 'r', encoding='utf-8')
# rdr = csv.reader(f)
# k = []
# for line in rdr:
#     k.append(line)
...
# k1 = []
# for line in k:
#     k1.append(('').join(line).strip())
#
#
# with open('/hdd/user4/bert_classification/agnews_data/agnews.test.all', 'w') as f:
#     for i in k1:
#         f.write(i)
#         f.write('\n')
